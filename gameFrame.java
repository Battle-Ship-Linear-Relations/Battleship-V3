...
public class gameFrame extends JFrame{ //extends JFrame so this class can just be treated as the JFrame
        ...
        this.addKeyListener(new KeyAdapter() { // add a key listener
            ...
            public void keyPressed(KeyEvent e) {
                if(e.getKeyCode() == KeyEvent.VK_R){
                    ...
                    if (shipCount == 0) ShipLength.shipLength4(userBoard, rotate, shipColour, mouseX, mouseY); //call the shipLength methods after the ship is rotated, so the ship is still visible after it is rotated
                    if (shipCount == 1) ShipLength.shipLength3(userBoard, rotate, shipColour, mouseX, mouseY);
                    if (shipCount == 2) ShipLength.shipLength2(userBoard, rotate, shipColour, mouseX, mouseY);
                    repaint();
                }
            }
        });
        ...
        userBoard[xCoord][yCoord].addMouseListener(new java.awt.event.MouseAdapter() {
            ...
            public void mouseEntered(java.awt.event.MouseEvent evt) {
                if (shipCount != 3) { // while the ship count is not equal to 3 (it is the user's turn), if not do nothing on hover afterwards
                    if (shipCount == 0) ShipLength.shipLength4(userBoard, rotate, shipColour, xCoord, yCoord); // when on hover, depending on the length of the ship being placed, change the colour of the JButtons to represent a ship
                    if (shipCount == 1) ShipLength.shipLength3(userBoard, rotate, shipColour, xCoord, yCoord);
                    if (shipCount == 2) ShipLength.shipLength2(userBoard, rotate, shipColour, xCoord, yCoord);
            ...
            public void mouseExited(java.awt.event.MouseEvent evt) {
                try {
                    ...
                    if (shipCount != 3) { // when the user has placed 3 ships, do nothing over hover, and whatever the button has been assigned set their fixed colour that can no longer change
                        for (int i = 0; i < userBoard.length; i ++) {
                            for (int j = 0; j < userBoard[0].length; j++) {
                                if(userBoard[i][j].getText().equals("0") || userBoard[i][j].getText().equals("1") || userBoard[i][j].getText().equals("2")) {
                                    userBoard[i][j].setBackground(Color.GREEN);
                                } 
                            }
                        }
                    }
    ...
    public void computerButtonPressed(int xCoord, int yCoord) { // if the robot is confirming its ships
        ...
        if (shipCount == 3) {
            if (ShipLength.shipLength4(computerBoard, rotate, shipColour, xCoord, yCoord)) {
                ShipLength.confirmShipLength4(computerBoard, rotate, shipColour, xCoord, yCoord, shipCount);
                shipCount++; // increase shipCount if the ship is placed without any errors
            }
        }
        else if (shipCount == 4) {
            if (ShipLength.shipLength3(computerBoard, rotate, shipColour, xCoord, yCoord)) {
                ShipLength.confirmShipLength3(computerBoard, rotate, shipColour, xCoord, yCoord, shipCount);
                shipCount++;
            }
        }
        else if (shipCount == 5) {
            if (ShipLength.shipLength2(computerBoard, rotate, shipColour, xCoord, yCoord)) {
                ShipLength.confirmShipLength2(computerBoard, rotate, shipColour, xCoord, yCoord, shipCount);
                shipCount++;
            }
        }
    ...
    public void buttonPressed(int xCoord, int yCoord) { // if the user if confirming a ship
        ...
        new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                if (shipCount == 0 && ShipLength.shipLength4(userBoard, rotate, shipColour, xCoord, yCoord)) {
                    ShipLength.confirmShipLength4(userBoard, rotate, shipColour, xCoord, yCoord, shipCount); // place a ship of the user's board and if successful, increase the shipcount, so the next length of ship will be placed
                    shipCount++;
                }
                else if (shipCount == 1 && ShipLength.shipLength3(userBoard, rotate, shipColour, xCoord, yCoord)) {
                    ShipLength.confirmShipLength3(userBoard, rotate, shipColour, xCoord, yCoord, shipCount);
                    shipCount++;
                }

                else if (shipCount == 2 && ShipLength.shipLength2(userBoard, rotate, shipColour, xCoord, yCoord)) {
                    ShipLength.confirmShipLength2(userBoard, rotate, shipColour, xCoord, yCoord, shipCount);
                    shipCount++;
                }
    ...
    public void confirmEquation() { // when enter is pressed on the coordinate field
        player player = new player(coordinateField, yIntTextField, riseTextField, runTextField);
            ...
            if (equationLabel.getText().equals("y < — x + o")) {
                for (int i = -5; i <= 5; i++) {
                    ...
                    for (int j = -5; j < endPoint; j++) {
                        try {
                            if (playerTurn % 2 == 0) {
                                computerBoard[middle + i][middle - j].setBackground(Color.PINK);
                            }
                            else {
